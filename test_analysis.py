...
def test_run_logistic_model_basic():
    ...
    df = pd.DataFrame(
        {"age": [40, 50, 60, 70], "sex_male": [1, 0, 1, 0], "disease": [0, 1, 1, 0]}
    )

    res = run_logistic_model(
        df, ["age", "sex_male"], target="disease", test_size=0.5, random_state=42
    )
...
def test_end_to_end_pipeline():
    ...
    df_d = pd.get_dummies(
        df, columns=["sex", "cp", "restecg", "fbs", "exang"], drop_first=True
    )
    ...
    res = run_logistic_model(
        df_d, subset, target="disease", test_size=0.2, random_state=123
    )
